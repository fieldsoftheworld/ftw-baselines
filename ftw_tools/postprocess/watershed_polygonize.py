...
def polygonize_with_watershed(
    ...
    close_interiors: bool = False,
    t_ext: float = 0.5,
    t_bound: float = 0.2,
):
    print(f"Polygonizing input file with watershed: {input}")
    ...
    print(f"Finished watershed polygonizing output at {out} in {time.time() - tic:.2f}s")


def polygonize_watershed_cli(
    input: str,
    out: str,
    simplify: float = 15,
    min_size: float = 500,
